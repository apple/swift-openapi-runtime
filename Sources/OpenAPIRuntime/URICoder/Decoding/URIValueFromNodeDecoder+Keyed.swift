    ...
    private func _decodeValue(forKey key: Key) throws -> URIParsedValue {
        guard let value = values[key.stringValue[...]]?.first else {
            throw DecodingError.keyNotFound(
                key,
                .init(codingPath: codingPath, debugDescription: "Key not found.")
            )
        }
        return value
    }
...
extension URIKeyedDecodingContainer: KeyedDecodingContainerProtocol {
    ...
    var allKeys: [Key] {
        values.keys.map { key in
            Key.init(stringValue: String(key))!
        }
    }
    ...
    func decode(_ type: String.Type, forKey key: Key) throws -> String {
        String(try _decodeValue(forKey: key))
    }

    func decode(_ type: Double.Type, forKey key: Key) throws -> Double {
        try _decodeBinaryFloatingPoint(forKey: key)
    }
    ...
    func decode(_ type: Int.Type, forKey key: Key) throws -> Int {
        try _decodeFixedWidthInteger(forKey: key)
    }
