...
struct LinesDeserializationSequence<Upstream: AsyncSequence & Sendable>: Sendable where Upstream.Element == ArraySlice<UInt8> {
...
extension LinesDeserializationSequence: AsyncSequence {
    ...
    struct Iterator<UpstreamIterator: AsyncIteratorProtocol>: AsyncIteratorProtocol where UpstreamIterator.Element == Element {
        ...
        var stateMachine: LinesDeserializerStateMachine = .init()
...
extension AsyncSequence where Element == ArraySlice<UInt8> {
    func asParsedLines() -> LinesDeserializationSequence<Self> {
        .init(upstream: self)
    }
}

struct LinesDeserializerStateMachine {
