...
public final class MultipartTypedBody<Part: MultipartTypedPart>: @unchecked Sendable {
    ...
    @inlinable public convenience init<Parts: AsyncSequence>(
        _ sequence: Parts,
        length: MultipartTypedBody.Length,
        iterationBehavior: IterationBehavior
    ) where Parts: Sendable, Parts.Element == Part {
        self.init(.init(sequence), length: length, iterationBehavior: iterationBehavior)
    }
}

extension MultipartTypedBody: Equatable {
    ...
    public static func == (lhs: MultipartTypedBody, rhs: MultipartTypedBody) -> Bool {
        ObjectIdentifier(lhs) == ObjectIdentifier(rhs)
    }
}

extension MultipartTypedBody: Hashable {
...
// MARK: - Creating the MultipartTypedBody

extension MultipartTypedBody {
    ...
    @inlinable public convenience init(
        _ parts: some Sequence<Part> & Sendable,
        length: Length,
        iterationBehavior: IterationBehavior
    ) { self.init(.init(WrappedSyncSequence(sequence: parts)), length: length, iterationBehavior: iterationBehavior) }
    @inlinable public convenience init(_ parts: some Collection<Part> & Sendable, length: Length) {
...
extension MultipartTypedBody: AsyncSequence {
...
extension MultipartTypedBody {
    ...
    public func collect(upTo maxBytes: Int) async throws -> [Part] {
...
extension MultipartTypedBody {
    ...
    @usableFromInline struct BodySequence: AsyncSequence, Sendable {
        ...
        @usableFromInline typealias AsyncIterator = MultipartTypedBody.Iterator

        /// The byte chunk element type.
        @usableFromInline typealias Element = Part
    ...
    where Bytes.Element == Part, Bytes.Iterator.Element == Part, Bytes: Sendable {
